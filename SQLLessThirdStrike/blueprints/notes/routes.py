...
def list_notes():
    ...
    if search_query:
        # Use regex for case-insensitive partial text search
        # $options: 'i' makes it case-insensitive
        # $regex: search_query allows partial matches
        query['text'] = {'$regex': search_query, '$options': 'i'}
        flash(f"Showing notes matching '{search_query}'", 'info')

    # --- Sorting functionality ---
    sort_by = request.args.get('sort_by', 'time_created') # Default sort by time_created
    sort_order = request.args.get('sort_order', 'desc') # Default sort order descending

    sort_direction = -1 if sort_order == 'desc' else 1

    # Define valid sort fields
    valid_sort_fields = ['time_created', 'text', 'appointed_time']
    if sort_by not in valid_sort_fields:
        sort_by = 'time_created' # Fallback to default if invalid sort_by is provided

    # Fetch notes with query and sort
    notes = notes_collection.find(query).sort(sort_by, sort_direction)

    return render_template(
        'notes/index.html',
        notes=notes,
        search_query=search_query, # Pass search query back to template
        sort_by=sort_by,           # Pass sort parameters back to template
...
# blueprints/notes/routes.py
@notes_bp.route('/bulk_actions', methods=['POST'])
def bulk_actions():
    selected_note_ids = request.form.getlist('selected_notes')
    action = request.form.get('action')

    if not selected_note_ids:
        flash('No notes selected for bulk action.', 'warning')
        return redirect(url_for('notes.list_notes'))

    # Convert string IDs to ObjectId
    object_ids = [ObjectId(id_str) for id_str in selected_note_ids]
    notes_collection = current_app.db.notes

    if action == 'delete':
        result = notes_collection.delete_many({'_id': {'$in': object_ids}})
        flash(f'{result.deleted_count} note(s) deleted successfully!', 'success')
    elif action == 'update_time':
        new_appointed_time_str = request.form.get('bulk_appointed_time')
        if not new_appointed_time_str:
            flash('No appointed time provided for bulk update.', 'danger')
            return redirect(url_for('notes.list_notes'))
        
        try:
            new_appointed_time = datetime.strptime(new_appointed_time_str, '%Y-%m-%dT%H:%M')
            result = notes_collection.update_many(
                {'_id': {'$in': object_ids}},
                {'$set': {'appointed_time': new_appointed_time}}
            )
            flash(f'{result.modified_count} note(s) updated with new appointed time!', 'success')
        except ValueError:
            flash('Invalid appointed time format for bulk update. Please use YYYY-MM-DDTHH:MM.', 'danger')
    elif action == 'clear_time':
        result = notes_collection.update_many(
            {'_id': {'$in': object_ids}},
            {'$unset': {'appointed_time': ''}} # Unset removes the field
        )
        flash(f'{result.modified_count} note(s) had their appointed time cleared!', 'success')
    else:
